...
* _Image filenames_ – phpThumbOf 1.4.0 has some problems with image names. In particular, with Hash Thumbnail Names set to off (the default, in System Settings under phpThumbOf) and multiple images with the same name (in different directories) on the same page getting called with the same phpThumbOf options, phpThumbOf will mistakenly use the first image for all the rest.  Also, if you have the exact same image used on different pages with the same phpThumbOf options it'll create separate files for each one, which means more work for the server and messes up caching in visitor's browsers.  See my [pull request](https://github.com/splittingred/phpThumbOf/pull/49) for a simple way to fix both problems.
...
* _Cache cleanup_ - phpThumb by default is set to clean up its cache directory: to delete files not accessed within a certain number of days, or delete the least recently used ones if the cache grows beyond a set number of images or megabytes. The only problem is it does this cleanup every time it's called, so if you have a page with 30 images it'll run its cleanup method 30 times while the page is being built, and that can start to affect performance.  If you'd like to disable this, go to System > System Settings and under core > phpThumb set Max Cache Age, Max Cache Files, and Max Cache Size (three settings) all to 0.  If you do this it'd be a good idea to periodically check the cache and make sure it hasn't grown to enormous size :-)
